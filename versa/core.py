...
from .errors import (InactiveUser, ObjectDoesNotExist, ResponseTookTooLong,
                     UserDoesNotExist)
from .utils import MockMember, issubmodule, titlecaseify
...
class Core(commands.Bot):
    ...
    def __init__(self, config, settings, name='default', loop=None):
        ...
        self.sync_db('versa', interactive=versa.TEST)
        ...
        self.sync_db(*extension_names, interactive=versa.TEST)
        ...
        user_agent = 'versa-framework (https://github.com/BezerraC/versa-framework {0}) ' \
                     'Python/{1} aiohttp/{2} discord.py/{3}'
        self.http.user_agent = user_agent.format(versa.__version__, sys.version.split(maxsplit=1)[0],
    ...
    @property
    @versa.cached(include_self=False)
    ...
    def _resolve_groups(self, cog_or_command):
        if isinstance(cog_or_command, versa.Cog):
    ...
    def load_extension(self, name):
        ...
        try:
            ...
            cog_module = importlib.import_module(f'versa.extensions.{name}.cogs')
            _local = False
        ...
        if name not in self.config.disabled_extensions:
            ...
            if hasattr(cog_module, 'setup'):
                ...
                cog_classes = inspect.getmembers(cog_module, lambda member: isinstance(member, type) and
                                                 issubclass(member, versa.Cog) and member is not versa.Cog)
    ...
    def _load_cogs(self):
        # discord and versa each have their own definition of an extension
    ...
    async def wait_for_response(self, ctx_or_message, responding=None, message_check=None, timeout=60, force_response=True) -> str:
        ...
        if isinstance(ctx_or_message, versa.Context):
    ...
    async def on_command_error(self, ctx: versa.Context, error):
        ...
        if isinstance(error, ObjectDoesNotExist) and not versa.TEST:
            model = error.model
