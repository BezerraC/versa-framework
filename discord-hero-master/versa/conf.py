...
class Extension:
    ...
    def get_controller(self, core):
        db = versa.Database(core)
        cache = versa.get_cache(self.name)
    ...
    def _controller_cls(self):
        try:
            ...
            try:
                controller_module = importlib.import_module(f'versa.extensions.{self.name}.controller')
            except ImportError:
                return None

        controller_class = inspect.getmembers(controller_module, lambda member: isinstance(member, type)
                                                                                and issubclass(member, versa.Controller)
                                                                                and member is not versa.Controller)
    ...
    def _settings_model(self):
        try:
            ...
            try:
                models_module = importlib.import_module(f'versa.extensions.{self.name}.models')
            except ImportError:
                return None

        from versa.models import CoreSettings, Settings
    ...
    def load_models(self):
        from versa.models import Model
        try:
    ...
    def get_extension_module(cls, name: str, local: bool):
        ...
        if local:
            spec = spec_from_file_location(f'extensions.{name}',
                                           os.path.join(versa.ROOT_DIR, 'extensions',
                                                        name, '__init__.py'))
        else:
            spec = spec_from_file_location(f'versa.extensions.{name}',
                                           os.path.join(versa.LIB_ROOT_DIR, 'extensions',
                                                        name, '__init__.py'))
...
class Config:
    ...
    @property
    def file_path(self):
        return os.path.join(versa.ROOT_DIR, self.file_name)
...
def get_extension_config(extension_name, local=False):
    ...
    try:
        ...
        raise ConfigurationError(f"extension {extension_name} doesn't have a versa.ExtensionConfig subclass")
